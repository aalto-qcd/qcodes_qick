...
class CMPGProtocol(Protocol):
    """
        This protocol initializes and runs a CMPG pulse sequence 
        program, and correctly formats the output into a desired form. 
    """
    def __init__(self, name="CMPG_Protocol"):
        """
        ...
        self.add_parameter('readout_pulse_delay',
                            ...
                            unit = 'us',
                            initial_value = 0.05)        
        self.add_parameter('n_pi_pulses',
                            parameter_class=ManualParameter,
                            label='Number of pi pulses in the CMPG pulse sequence',
                            vals = Ints(1,100),
                            initial_value = 1)   
    ...
    def initialize_qick_config(self, sweep_configuration):
        ...
        external_parameters = { 
                    ...
                    'pi_pulse_count' : self.n_pi_pulses,

                    #For the CMPG program, readout probe pulse
    ...
    def run_program(self, soc, cfg : Dict[str, float]):
        ...
        expt_pts, avg_i, avg_q = self.run_hybrid_loop_program(soc, self.cfg, CMPGProgram)
...
class CMPGProgram(RAveragerProgram):
    """
    ...
    def initialize(self):
        ...
        self.regwi(self.q_rp, self.r_variable_delay, self.us2cycles(cfg["start"] / (2*cfg["pi_pulse_count"] ))) #divide by two since variable delay will represent the symmetric wait time between the three pulses
    ...
    def body(self):
        ...
        # Wait variable time tau / (pi_pulse_count*2) 
        self.sync_all()
        self.sync(self.q_rp, self.r_variable_delay)
        
        for i in range(self.cfg["pi_pulse_count"] - 1):
            ...
            # Wait variable time tau / (pi_pulse_count)
            self.sync_all()
            self.sync(self.q_rp, self.r_variable_delay)
            self.sync(self.q_rp, self.r_variable_delay)

        #play last pi pulse and wait tau/(2*pi_pulse_count)
        self.pulse(ch=self.cfg["qubit_ch"])  
        self.sync(self.q_rp, self.r_variable_delay)
    ...
    def update(self):
        self.mathi(self.q_rp, self.r_variable_delay, self.r_variable_delay, '+', self.us2cycles(self.cfg["step"] / (2*self.cfg["pi_pulse_count"]) )) # update the time between two π/2 pulses
