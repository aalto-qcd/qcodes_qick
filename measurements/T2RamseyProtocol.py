...
class T2RamseyProtocol(Protocol):
    ...
    def run_program(self, soc, cfg : Dict[str, float]):
        ...
        expt_pts, avg_i, avg_q = self.run_hybrid_loop_program(soc, self.cfg, T2_Ramsey_Program)
...
class T2_Ramsey_Program(RAveragerProgram):
    def initialize(self):
        cfg=self.cfg
        ...
        self.regwi(self.q_rp, self.r_variable_delay, self.us2cycles(cfg["start"]))
    ...
    def update(self):
        self.mathi(self.q_rp, self.r_variable_delay, self.r_variable_delay, '+', self.us2cycles(self.cfg["step"])) # update the time between two π/2 pulses
