...
class PulseProbeSpectroscopyProtocol(Protocol):
    ...
    def run_program(self, soc, cfg : Dict[str, float]):
        ...
        """
        expt_pts, avg_i, avg_q = self.run_hybrid_loop_program(soc, cfg, PulseProbeSpectroscopyProgram)
...
class PulseProbeSpectroscopyProgram(RAveragerProgram):
    ...
    def initialize(self):
        ...
        probe_freq = self.freq2reg(cfg["start"], gen_ch=qubit_ch)
        ...
        self.declare_gen(ch=cfg["cavity_ch"], nqz=cfg["cavity_nqz"], ro_ch=cfg["ro_ch"]) #Cavity resonant probe
